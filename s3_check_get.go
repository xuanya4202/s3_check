...
func GetObject(sess *s3.S3, bucket string, objectname string, size uint64)(ok bool, err error){
	...
	var needread uint64 = size
	for{
		if(needread == 0){
			fmt.Println(objectname, needread)
			return true,nil
		}
		var len uint64 = 0
		if(needread > 1024*128){
			len = 1024*128
		}else{
			len = needread
		}
		bytesRead,readErr := resp.Body.Read(body[0:len])
		if readErr != nil{
			if readErr == io.EOF {
				needread = needread - uint64(bytesRead)
				break
			}
			return false, readErr
		}
	//	fmt.Println(objectname, bytesRead, needread)
		needread = needread - uint64(bytesRead)
	//	fmt.Println(objectname, bytesRead, needread)
	}
	...
	if(needread == 0){
		...
		err := errors.New(fmt.Sprintf("total size %v, read size %v", size, size-needread))
		return false,err
	}
}
...
func CurrentDir(Sess1 *SessInfo, DirNum *sync.WaitGroup, Prefix string, PrefixCh chan string, Sess1Bucket string){
	...
	for name,value := range Object1Map{
		fmt.Println(name)
		ok,err := GetObject(Sess1.S3, Sess1Bucket, name, value.size)
		if !ok {
			Sess1.ObjMu.Lock()
			_,err := Sess1.ObjFile.WriteString(name+"####"+err.Error()+ "\n")
			if nil != err{
				panic(err)
			}
			Sess1.ObjMu.Unlock()
...
func main() {	
	...
	flag.IntVar(&GoNum, "n", 2, "Goroutines num.")
